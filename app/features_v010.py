...
def configure(app, server):
    ...
    def current_event(conn, sold_at=None):
        manual = conn.execute('SELECT * FROM events WHERE manual_active=1 ORDER BY id DESC LIMIT 1').fetchone()
        if manual:
            return manual
        d = (sold_at or datetime.now()).date().isoformat() if isinstance(sold_at, datetime) else date.today().isoformat()
        return conn.execute('SELECT * FROM events WHERE start_date<=? AND end_date>=? ORDER BY id DESC LIMIT 1', (d, d)).fetchone()
    ...
    # ---- Inventory search: exact physical barcode jumps straight to the product. ----
    def inventory_index():
    ...
    def toggle_barcode(barcode_id):
        with db() as conn:
            ...
            if b['state'] == 'Available': record_sale(conn, b, 'Manual')
            else: record_return(conn, b)
        return redirect(url_for('product_detail', product_id=product_id))
    ...
    def toggle_all(product_id):
        with db() as conn:
            ...
            if available:
                for b in available: record_sale(conn, b, 'Manual')
            else:
                for b in rows: record_return(conn, b)
        return redirect(url_for('product_detail', product_id=product_id))
    ...
    def sales_history():
        ...
        with db() as conn:
            ...
            params = []
            if event_id.isdigit(): sql += ' AND s.event_id=?'; params.append(int(event_id))
    ...
    def events():
        if request.method == 'POST':
            ...
            start = request.form.get('start_date','').strip(); end = request.form.get('end_date','').strip()
            if not name or not start or not end:
                flash('Name, start date and end date are required.', 'error')
            else:
                with db() as conn:
                    cur = conn.execute('INSERT INTO events(name,location,start_date,end_date,notes,created_at) VALUES(?,?,?,?,?,?)',
    ...
    def end_event(event_id):
        with db() as conn:
            event = conn.execute('SELECT * FROM events WHERE id=?', (event_id,)).fetchone()
            if not event: abort(404)
            conn.execute('UPDATE events SET manual_active=0 WHERE id=?', (event_id,))
    ...
    def delete_event(event_id):
        with db() as conn:
            ...
            if used:
                ...
                conn.execute('DELETE FROM events WHERE id=?', (event_id,)); log(conn, 'Event', f'Event #{event_id} deleted')
        return redirect(url_for('events'))

    # ---- Activity log + small Undo. ----
    @app.get('/activity')
    def activity():
        with db() as conn:
    ...
    def undo_activity(log_id):
        with db() as conn:
            ...
            if not row or not row['undo_kind'] or row['undone_at']:
                flash('That action cannot be undone.', 'error'); return redirect(request.referrer or url_for('activity'))
    ...
    def restore_archive(product_id):
        with db() as conn:
            p = conn.execute('SELECT * FROM products WHERE id=?', (product_id,)).fetchone()
            if not p: abort(404)
            conn.execute('UPDATE products SET archived_at=NULL WHERE id=?', (product_id,)); log(conn,'Archive',f"Restored {p['item']}",product_id)
        return redirect(url_for('product_detail', product_id=product_id))
    ...
    def permanent_delete(product_id):
        ...
        with db() as conn:
            ...
            conn.execute('DELETE FROM products WHERE id=?', (product_id,)); log(conn,'Delete',f"Permanently deleted {p['item']}")
        for ph in photos:
            for name in (ph['filename'], ph['original_filename']):
                if name:
                    try: (PHOTO_DIR / name).unlink(missing_ok=True)
                    except OSError: pass
        return redirect(url_for('index'))
    ...
    @app.get('/backups')
    def backups():
        files = sorted([p for p in BACKUP_DIR.glob('*.db')], key=lambda p:p.stat().st_mtime, reverse=True)
    ...
    def backup_create():
        ...
        with db() as conn: log(conn,'Backup',f"Backup created: {target.name}")
        flash('Backup created.', 'success'); return redirect(url_for('backups'))
    ...
    def backup_restore(name):
        ...
        if not source_path.exists(): abort(404)
        make_backup('before-restore')
        source = sqlite3.connect(source_path); dest = sqlite3.connect(DB_PATH)
        try: source.backup(dest)
        finally: dest.close(); source.close()
        flash('Backup restored. The page will now use the restored inventory.', 'success')
    ...
    # ---- Photo management: download, main photo, crop only. ----
    ...
    def main_photo(photo_id):
        with db() as conn:
            ...
            for i,r in enumerate(others,2): conn.execute('UPDATE photos SET sort_order=? WHERE id=?',(i,r['id']))
    ...
    def crop_photo(photo_id):
        try:
            x=float(request.form.get('x','0')); y=float(request.form.get('y','0')); w=float(request.form.get('w','1')); h=float(request.form.get('h','1'))
        except ValueError:
            flash('Crop values were invalid.', 'error'); return redirect(request.referrer or url_for('index'))
        x=max(0,min(1,x)); y=max(0,min(1,y)); w=max(.02,min(1-x,w)); h=max(.02,min(1-y,h))
        with db() as conn:
            ph=conn.execute('SELECT * FROM photos WHERE id=?',(photo_id,)).fetchone()
            if not ph: abort(404)
            original_name=ph['original_filename']
            if not original_name:
                src=PHOTO_DIR/ph['filename']; ext=src.suffix.lower() or '.jpg'; original_name=f"originals/original_{photo_id}_{uuid.uuid4().hex}{ext}"
                (PHOTO_DIR/original_name).parent.mkdir(parents=True,exist_ok=True); shutil.copy2(src,PHOTO_DIR/original_name)
                conn.execute('UPDATE photos SET original_filename=? WHERE id=?',(original_name,photo_id))
            source=PHOTO_DIR/original_name; target=PHOTO_DIR/ph['filename']
            with Image.open(source) as img:
                iw,ih=img.size; box=(round(x*iw),round(y*ih),round((x+w)*iw),round((y+h)*ih)); cropped=img.crop(box)
                if cropped.mode not in ('RGB','RGBA') and target.suffix.lower() in ('.jpg','.jpeg'): cropped=cropped.convert('RGB')
                cropped.save(target)
    ...
    # Override PayPal import so state changes create permanent sales/returns and inherit active event.
    def paypal_import():
        uploaded=request.files.get('file')
        if not uploaded or not uploaded.filename:
            flash('Choose a PayPal stock CSV first.','error'); return redirect(url_for('paypal'))
        try:
            reader=csv.DictReader(io.StringIO(uploaded.read().decode('utf-8-sig')))
        except Exception as exc:
            flash(f'Could not read CSV: {exc}','error'); return redirect(url_for('paypal'))
        ...
        with db() as conn:
            for row in reader:
                ...
                if not code or qty_text=='': skipped+=1; continue
                try: qty=int(float(qty_text))
                ...
                if b['state']!=desired:
                    if desired=='Sold': record_sale(conn,b,'PayPal POS'); sales_count+=1
                    else: record_return(conn,b,'PayPal POS')
                updated+=1
    ...
    # Make the product detail page expose event-aware sales count without changing old data model.
    original_detail=app.view_functions.get('product_detail')

    # Log new barcode additions and product edits by wrapping selected existing handlers lightly.
    original_add=app.view_functions.get('add_item_barcode')
    if original_add:
        def add_item_barcode_logged(product_id):
            code=request.form.get('barcode','').strip(); response=original_add(product_id)
            if code:
                try:
                    with db() as conn:
                        b=conn.execute('SELECT * FROM item_barcodes WHERE barcode=? AND product_id=?',(code,product_id)).fetchone()
                        if b: log(conn,'Barcode',f"Barcode {code} added",product_id,code)
