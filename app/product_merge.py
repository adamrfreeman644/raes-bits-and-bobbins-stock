    ...
    def merge_products(product_id):
        ...
        with db() as conn:
            ...
            for source_id in source_ids:
                ...
                if not source:
                    abort(404)
                if signature(conn, source) != expected:
                    flash(f'Product #{source_id} no longer exactly matches. Nothing was merged.', 'error')
                    return redirect(url_for('merge_product_picker', product_id=product_id))
            ...
            for source in sources:
                ...
                # Re-point historical tables when present. The barcode rows themselves keep their IDs,
                # so sales.barcode_id remains valid.
        ...
        flash(
            f'Merged {len(source_ids)} duplicate product record(s). '
            f'{moved_barcodes} barcodes and {moved_photos} photos were preserved.',
            'success',
        )
